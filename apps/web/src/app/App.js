...
import { SargassumSirMap } from "../components/forecast/SargassumSirMap";
import { LandfallForecastPage } from "../components/landfall/LandfallForecastPage";
...
export function App() {
    ...
    useEffect(() => {
        ...
        getBeaches(selectedCounty === "All" ? undefined : selectedCounty)
            ...
            setBeaches(rows);
            // Preserve a deliberate selection when it remains visible in the new filter.
            // Otherwise, fall back to the first beach so the detail panel stays populated.
            const nextId = rows.some((beach) => beach.id === selectedBeachId) ? selectedBeachId : rows[0]?.id;
