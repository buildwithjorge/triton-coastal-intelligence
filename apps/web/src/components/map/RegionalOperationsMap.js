...
export function RegionalOperationsMap({ beaches, selectedBeachId, selectedCounty, onCountyChange, onBeachSelect }) {
    ...
    const [hoveredBeachId, setHoveredBeachId] = useState(undefined);
    ...
    return (_jsxs("section", { className: "panel rounded-2xl p-4", children: [_jsxs("div", { className: "mb-4 flex flex-wrap items-center justify-between gap-3", children: [_jsxs("div", { children: [_jsx("p", { className: "text-xs uppercase tracking-[0.2em] text-teal", children: "Regional Coastal Operations Map" }), _jsx("h2", { className: "text-xl font-semibold", children: "Jupiter to Key Biscayne Live Coverage" })] }), _jsxs("div", { className: "flex gap-2", children: [_jsx("button", { onClick: () => onCountyChange("All"), className: `rounded-md border px-3 py-1 text-xs font-medium uppercase tracking-[0.16em] ${selectedCounty === "All" ? "border-teal bg-teal/20 text-teal" : "border-border bg-panel-hi/70 text-steel"}`, children: "All" }), counties.map((county) => (_jsx("button", { onClick: () => onCountyChange(county), className: `rounded-md border px-3 py-1 text-xs font-medium uppercase tracking-[0.16em] ${selectedCounty === county ? "border-teal bg-teal/20 text-teal" : "border-border bg-panel-hi/70 text-steel"}`, children: county }, county)))] })] }), _jsx("canvas", { ref: canvasRef, width: canvasWidth, height: canvasHeight, className: "w-full rounded-xl border border-border bg-panel-hi/40", onMouseMove: (event) => {
                ...
                }, onMouseLeave: () => setHoveredBeachId(undefined), onClick: (event) => {
                    const rect = event.currentTarget.getBoundingClientRect();
                    const scaleX = canvasWidth / rect.width;
                    const scaleY = canvasHeight / rect.height;
                    const x = (event.clientX - rect.left) * scaleX;
                    ...
                    if (selected)
                        onBeachSelect(selected.beachId);
                } }), _jsxs("div", { className: "mt-3 grid grid-cols-2 gap-2 text-xs text-steel md:grid-cols-4", children: [_jsx("span", { className: "font-mono text-green", children: "LOW" }), _jsx("span", { className: "font-mono text-amber", children: "MODERATE" }), _jsx("span", { className: "font-mono text-orange", children: "HEAVY" }), _jsx("span", { className: "font-mono text-red", children: "SEVERE" })] })] }));
}
