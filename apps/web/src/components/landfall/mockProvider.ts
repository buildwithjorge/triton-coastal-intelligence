...
import type { LandfallForecast, SargassumForecastDataset, SargassumPatch } from "./types";

type ForecastTemplate = Omit<LandfallForecast, "targetBeachIds" | "isCoverageArea"> & {
  targetBeachNames?: string[];
};

const mockPatches: SargassumPatch[] = [
  {
    id: "patch-a",
...
function resolveCoverageBeachIds(beachByName: Map<string, number>, names?: string[]): number[] | undefined {
...
export class MockForecastProvider implements SargassumForecastProvider {
  async getDataset(): Promise<SargassumForecastDataset> {
    let coverageBeachIds = new Set<number>();
    let beachByName = new Map<string, number>();
    ...
    try {
      ...
      coverageBeachIds = new Set<number>();
      beachByName = new Map<string, number>();
    }

    const forecasts: LandfallForecast[] = forecastTemplates.map((template) => {
