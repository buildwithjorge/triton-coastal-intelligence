...
export function projectLonLat(lon, lat, width, height, bounds = defaultBounds) {
...
export function blobPath(cx, cy, baseRadius, seed) {
    const points = [];
    const vertices = 9;
...
export function ribbonPath(points, startWidth, endWidth) {
    if (points.length < 2)
        return "";
    const left = [];
    const right = [];
...
export function pathProgressForHorizon(horizon, arrivalMax) {
    if (arrivalMax <= 0)
        return 0;
